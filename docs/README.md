...
|---|---|---|---|---|---|---|
| `README.md` | tutorial | 项目维护者；首次运行入口 | `pyproject.toml`、`deploy/compose.yaml`、CLI | 2026-07-26 | 每次里程碑 | 安装、命令或实现阶段变化 |
| `docs/architecture.md` | explanation；architectural | 架构维护者；理解控制流和数据流 | `src/pystream/control`、`runtime`、`worker` | 2026-07-26 | 每次模块边界变化 | 拓扑、协议、失败语义变化 |
| `docs/modules.md` | explanation | 各模块修改者；定位责任 | `src/pystream/` | 2026-07-26 | 每次顶层模块变化 | 新模块缺失或依赖方向变化 |
| `docs/api.md` | reference | API 修改者；编写作业 | Pydantic 模型、协议模型、CLI parser | 2026-07-26 | 每次公共契约变化 | YAML、UDF、CLI 或帧字段变化 |
| `docs/deployment.md` | how-to；operational | 部署执行者；启动和验收 | Dockerfile、Compose、`scripts/` | 2026-07-26（静态；Docker E2E 待验证） | 每次部署资产变化 | 镜像、端口、卷、脚本变化或 E2E 失败 |
| `docs/testing.md` | how-to | 开发者；执行质量门 | `pyproject.toml`、`tests/` | 2026-07-26 | 每次质量命令变化 | CI/本地命令或门槛变化 |
...
| `docs/troubleshooting.md` | how-to；operational | 部署执行者；定位失败 | 健康/状态 API、JSON 日志事件 | 2026-07-26 | 每次故障处理变化 | 错误类型、日志字段或端点变化 |
| `docs/roadmap.md` | explanation | 规格维护者；判断当前语义边界 | 已批准规格、状态/Sink 扩展接口 | 2026-07-26 | 每阶段开始/结束 | 实现能力与阶段标签不一致 |
...
| 主题 | 当前事实 | 验证 |
|---|---|---|
| 拓扑 | 单 JobManager、3 Worker、单 Kafka KRaft Broker | Compose 静态契约测试；Docker E2E 待执行 |
...
| 恢复 | 第一阶段遇故障整作业失败，不自动恢复 | 控制面与运行时测试 |
| 规划能力 | Watermark、Retract、Checkpoint、事务 Sink 尚未实现 | `docs/roadmap.md` 明确标记为 planned |

部署文档在 Docker 实机验收前保持“静态已验证、运行待验证”状态，不能改成“完整
通过”。完成 E2E 后应记录日期、Docker 版本和验证结果。
...
5. 当前为什么不具备 At-least-once/Exactly-once。
